...
public class SeleniumHomePageTest {
    ...
    @Test
    @DisplayName("Test home page vets link")
    public void takeVetsServiceSnapshot(TestInfo testInfo) throws Exception {
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        WebElement billsLink = helper.getDriver().findElement(By.id("HomeVetLink"));
        billsLink.click();

        WebDriverWait wait = new WebDriverWait(driver, 2);

        /* TODO: Check titles if VETS ever puts IDs
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("BillHistoryTitle")));
